...
`compat.css` covers the known spots where the app hard-codes colours (left menu, icons, Angular Material radio
buttons, ag-Grid lists, tab strips, dialogs). On top of that the extension runs a small sweep after each render:
any element still painted with a light background, dark text or a dark icon once the theme has applied is marked
and recoloured with tokens. That is what makes the assistant (Micro), the report picker and other screens we never
looked at follow along. Chart grid lines and anything else drawn on a canvas cannot be restyled from CSS.

After changing files here, reload the extension on chrome://extensions (the circular arrow on its card) and refresh Unimicro.
